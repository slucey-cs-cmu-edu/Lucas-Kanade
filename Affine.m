% Object class for 2D Affine warps to be used in conjunction with the LK class  
%
% Written by Simon Lucey 2015
classdef Affine        
    % Define constant properties
    properties(Constant)
        P = 6; % Degrees of freedom in the warp
    end
    properties
    end
     methods
        %% ----------------------------------------------------------------
        % Constructor function for the Affine Warp
        function o = Affine(varargin)
        ...
        %% ----------------------------------------------------------------
        % The derivative of the warp function for an affine warp w.r.t p
        %
        % Usage:- [dWx,dWy] = o.dWx_dp(p)
        %
        % <input>   
        %           x = x-coordinates 
        %           p = input warp vector (not needed for the Affine warp)
        %
